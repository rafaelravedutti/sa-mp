/*-----------------------------------------------------------
	Virtual Society RPG
	Copyright(C) 2010/2011 Virtual Society Team
...
#define JOB_MACHINIST               	7
#define JOB_TRUCKER                     8
#define JOB_JOURNALIST                  9
#define JOB_MECHANIC                    10
#define JOB_PARAMEDIC                   11
#define JOB_POLICE			            12
#define JOB_LAWYER                      13
#define JOB_SWAT 		         		14
#define JOB_DRUG_DEALER            		15
#define JOB_DELEGATE               		16
#define JOB_JUDGE                       17
...
#define INT_HOTEL                       26
#define INT_ACCESS1                  	27
#define INT_ACCESS2                     28
#define INT_ACCESS3                     29
#define INT_ACCESS4                     30
#define INT_ACCESS5                     31
...
#define COMPANY_NEWS        			8
#define COMPANY_CUSTOM    				9
#define COMPANY_TRANSPORT   			10
#define COMPANY_TRUCK       			11
#define COMPANY_CRACK       			12
#define COMPANY_LAWYER      			13
#define COMPANY_TRIBUNAL    			14
...
new Areas[][areaInfo] = {
	...
	{"Pesca", 				AREA_FISH,          	15.0,       407.8179,		-2088.7974,		7.8359, 	9}
};
...
//Descrições
	...
	{"", "Esteja na Delegacia para checar a ficha criminal de alguém, talvez poderá lhe ajudar nos argumentos.",                            {JOB_LAWYER}},
...
new Companies[][companyInfo] = {
	NULL,
	{"Pescaria",            			"Cooperativa de Pesca",  	"Fish",				75000,         	JOB_FISHER,               	408.9590,    	-2087.7498,    	7.8359},
	...
	{"Hospital",						"Anjos da Guarda",         	"Hospital",			0,				JOB_PARAMEDIC,         		2036.6701,    	-1409.8912,    	17.1640},
	{"Delegacia",	      				"LSPD",						"Police",			0,				JOB_POLICE, 		  		1545.6605,    	-1675.6268,    	13.5607},
	{"Noticiario",                      "Por Dentro",       		"News",             120000,         JOB_JOURNALIST,             732.8124,    	-1358.3757,    	23.5792,    58},
	{"Oficina",			    			"Inovacao",					"Custom",			75000,			JOB_MECHANIC,               2131.8483,    	-1150.6437,    	24.1351},
	{"Transportes Publicos",  			"Carona do Estado",			"Transport",		0,              JOB_MACHINIST,   			1753.6323,    	-1894.3092,    	13.5571,	56},
	...
	{"Advocacia",   					"Apoio Judicial",			"Lawyer",			140000,         JOB_LAWYER,              	1207.4970,    	-1438.9748,    	13.3828,	60},
...
new Enters[][enterInfo] = {
    ...
    {INT_BANK,                  2303.6652,    	-16.1838,    	26.4843,        7},
...
new Weathers[][weatherInfo] = {
	...
	{8,			"Tempestade, abaixo de 20°C",		"Dia bom para ficar em casa embaixo dos cobertores assistindo televisão, nada de sair."},
	...
	{16,		"Chuva, cerca de 22°C",				"A água está por vir, se for sair, não esqueça o guarda-chuva. ^^"},
	{17,    	"Ensolarado, cerca de 35°C",        "Dia muito quente, ótimo para beber um refrigerante e ir a praia."},
	{19,    	"Tempestade de areia, 40°C",        "Tempestade de areia, salve-se quem puder, ainda mais com o calor."},
	{20,		"Neblina, cerca de 20°C",			"Muita neblina vem por ai, aguardem... ^^"},
	{40,		"Céu limpo, cerca de 23°C",			"Temperatura estabilizada, céu limpo, sem registro de chuvas."},
	{45,		"Céu limpo, cerca de 19°C",			"Uma temperatura consideravelmente boa... Apesar do frio."}
};
...
stock ShowVehicleDocumentsForPlayer(vehicleid, playerid, driverid)
{
    ...
    if(IsServerVehicle(vehid))
	{
		...
		onsale = ("Não");
		fine = 0;
		format(plate, sizeof plate, "PSR %d", vehid + 1000);
	}
...
stock GetPlateVehicleId(const plate[])
{
	...
	for(new v = 0; v < GetVehicles(); v++)
	{
	    if(IsServerVehicle(v))
	        format(vplate, sizeof vplate, "PSR %d", v + 1000);
...
stock AddStaticServerVehicle(modelid, Float:X, Float:Y, Float:Z, Float:ang, colour1, colour2, respawn_delay, virtual_world = 0)
{
	...
	format(plate, sizeof plate, "PSR %d", id + 1000);
...
stock CreatePlayerVehicle(const owner[], modelid, Float:X, Float:Y, Float:Z, Float:ang, colour1, colour2)
{
	...
	format(plate, sizeof plate, "PSR %d", sysid + 2000);
...
stock ShowTuningMainMenu(playerid)
{
	...
	if(TunnerID != playerid)
	{
		...
		{
		    DrawTextForPlayer(playerid, "Já há alguem modificando um veículo.", COLOUR_RED, TEXTDRAW_STYLE_3, 3000);
		    return 1;
		}
	 		...
	 		modelid == 501 || modelid == 465 || modelid == 564	)
		{
  		...
  		for(new n = 0; n < MAX_SLOTS; n++)
  		{
			if(n != playerid && IsPlayerInVehicle(n, vehid))
			{
			    new Float:A;
...
stock ShowPaintjobCostMenu(playerid, paintjobid)
{
    ...
    mCost = CreateMenu("Pintura", 2, 20, 150, 100, 20);
    SetMenuColumnHeader(mCost, 0, "Pintura");
	SetMenuColumnHeader(mCost, 1, "Valor:");
    AddMenuItem(mCost, 0, "Comprar");
    AddMenuItem(mCost, 1, "$1000");
    AddMenuItem(mCost, 0, "Cancelar");
...
stock ShowTuningCostMenu(playerid, componentid)
{
	...
	mCost = CreateMenu("Componente", 2, 20, 150, 100, 20);
	SetMenuColumnHeader(mCost, 0, Components[componentid - 1000][tuneName]);
	SetMenuColumnHeader(mCost, 1, "Valor:");
	AddMenuItem(mCost, 0, "Comprar");
...
stock Employ(playerid, company, post, reward, payday)
	hSetInt(COMPANIES_FILE_PATH, PlayerName(playerid), ((company & 0xF) << 24) | ((post & 0xF) << 20) | ((reward & 0xFFF) << 8) | ((payday / 100) & 0xFFF));
...
public OnGameModeInit()
{
	...
	fcreate(JOBS_REPORT_FILE_PATH);
	fcreate(POLICE_REPORT_FILE_PATH);
	...
	//Auto Escola - Carros
	DRIVING_SCHOOL_CAR[0] = AddStaticServerVehicle(410, 1778.4686, -2076.3037, 13.2469, 90.6574, 3, 3, 1500);
	DRIVING_SCHOOL_CAR[1] = AddStaticServerVehicle(410, 1766.9196, -2076.3057, 13.2610, 90.4201, 3, 3, 1500);
	DRIVING_SCHOOL_CAR[2] = AddStaticServerVehicle(410, 1769.3566, -2022.6123, 13.7949, 271.5938, 3, 3, 1500);
	DRIVING_SCHOOL_CAR[3] = AddStaticServerVehicle(410, 1784.2090, -2041.7047, 13.1850, 270.2375, 3, 3, 1500);
	DRIVING_SCHOOL_CAR[4] = AddStaticServerVehicle(521, 1773.0260, -2061.4858, 13.1724, 0.2121, 3, 3, 1500);
	DRIVING_SCHOOL_CAR[5] = AddStaticServerVehicle(521, 1765.9749, -2041.0818, 13.1276, 270.1337, 3, 3, 1500);
	DRIVING_SCHOOL_CAR[6] = AddStaticServerVehicle(410, 1751.5083, -2056.9053, 13.4299, 179.9104, 3, 3, 1500);
	DRIVING_SCHOOL_CAR[7] = AddStaticServerVehicle(410, 1802.9364, -2064.8848, 13.2192, 179.6280, 3, 3, 1500);
	DRIVING_SCHOOL_CAR[8] = AddStaticServerVehicle(410, 1798.5068, -2064.8064, 13.2257, 179.8395, 3, 3, 1500);
	
	//Auto Escola - Avião
	DRIVING_SCHOOL_PLANE[0] = AddStaticServerVehicle(519, 1692.2711, -2451.8022, 14.4767, 182.7362, 0, 0, 1500);
	
	//Auto Escola - Barco
	DRIVING_SCHOOL_BOAT[0] = AddStaticServerVehicle(493, 728.0335, -1495.4415, -0.0825, 181.0754, 3, 3, 1500);
    ...
    printf("Veiculos carregados [Servidor: %d] [Jogadores: %d] [A venda: %d] [Total: %d]", MaxServerVeh, MaxPlayerVeh, sizeof VehiclesForSale, GetVehicles());
	...
	Guns[6] = CreateMenu("Explosivos", 2, 20, 150, 100, 40);
	...
	Guns[7] = CreateMenu("Outros", 2, 20, 150, 100, 40);
    ...
    TextDrawLetterSize(tdphone, 0.5, 1.0);
...
public StartGameModeExit()
{
    ...
    SetTimerEx("TurnGameModeOff", 10000, false, "d", _:textend);
}
...
stock ShowLoginDialog(playerid)
{
	if(fexist(GetPlayerXFilePath(playerid)))
	    ShowPlayerDialog(playerid, DIALOG_REGISTER_LOGIN, DIALOG_STYLE_PASSWORD, "Virtual Society RPG", "Bem-vindo, entre com a sua senha para continuar:", "Autenticar", "Cancelar");

	else
	    ShowPlayerDialog(playerid, DIALOG_REGISTER_LOGIN, DIALOG_STYLE_PASSWORD, "Virtual Society RPG", "Bem-vindo, registre uma senha para continuar:", "Registrar", "Cancelar");
}

stock ShowLotteryDialog(playerid)
    ShowPlayerDialog(playerid, DIALOG_LOTTERY, DIALOG_STYLE_INPUT, "Loteria", "Digite os seus numeros de aposta:\n\nExemplo: 10 20\n\nNumeros de 1 a 30", "Apostar", "Cancelar");
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	{
	    TextDrawHideForPlayer(playerid, tdctm[playerid]);
...
public OnPlayerSelectedMenuRow(playerid, row)
{
	...
	if(mid == mCurrentTune)
	{
	    if(RowComponents[row] == 500)
		{
	    	...
	    	mCurrentTune = CreateMenu("Componentes", 2, 20, 150, 100, 20);
	...
	else if(mid == mCost)
	{
	    switch(row)
	    {
	        case 0:
	        {
				...
				{
				    if(GetPlayerSysMoney(playerid) < 1000)
				    {
				        DrawTextForPlayer(playerid, "Dinheiro insuficiente.", COLOUR_RED, TEXTDRAW_STYLE_3, 1500);
				        ShowTuningMainMenu(playerid);
				        return 1;
				    ...
				    GivePlayerSysMoney(playerid, -1000);
				    ShowTuningMainMenu(playerid);
	            	TogglePlayerControllable(playerid, false);
	            	DrawTextForPlayer(playerid, "Pintura aplicada.", COLOUR_GREEN, TEXTDRAW_STYLE_2, 1000);
				    return 1;
				}
	        }
...
stock UpdatePlayerCrouch(playerid)
{
    if(GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_DUCK)
    {
        if(CarryObject[playerid] != -1)
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	if(newkeys & K_SPEAK && Radio[playerid] != -1)
	{
		...
		{
		    tdspeak[playerid] = TextDrawCreate(20.0, 106.0, Speaks[SpeakId[playerid]]);
		    TextDrawColour(tdspeak[playerid], COLOUR_WHITE);
		    TextDrawFont(tdspeak[playerid], 1);
		    TextDrawLetterSize(tdspeak[playerid], 0.51, 1.4);
	...
	if(PressedKey(K_ROPE, newkeys, oldkeys) && GetPlayerState(playerid) == PLAYER_STATE_PASSENGER)
	{
	    ...
	    if(IsJobVehicle(vehid, POLICE_CAR) && model == 497)
	    {
			...
			for(new r = 0; r < ROPES; r++)
			{
			    ...
			    Z -= ROPE_HEIGHT;
			    Ropes[playerid][r] = CreateObject(3004, X, Y, Z, 87.64, 342.135, 350.075);
			}
...
_loopstart:
        ...
        tdvinfo[playerid] = TextDrawCreate(219.0, 349.0, TEXT(string));
				...
				if(IsPlayerConnected(n) && IsPlayerLogged(n) && n != playerid)
				{
				    if(IsPlayerInVehicle(n, vehid) && GetPlayerState(n) == PLAYER_STATE_DRIVER && (hGetInt(GetPlayerXFilePath(n), "Job") == JOB_CABBIE || IsPlayerAdmin(n)) && Taximeter[n] == -1)
				    {
						...
						else
						{
						    ...
						    tdtaxi = TextDrawCreate(21.0, 125.0, TEXT("~y~Taxímetro: ~g~$~w~2,00 dólares"));
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if(strcmp(cmdtext, "/ajuda", true) == 0 || strcmp(cmdtext, "/help", true) == 0)
	{
		...
		SendClientMessage(playerid, COLOUR_SKY_BLUE, "/pagarfianca - /novidades");
	    return 1;
	}
	...
	if(strcmp(cmd, "/status", true) == 0)
	{
		...
		if(strlen(category) > 0)
		{
            ...
            else if(strcmp(category, "profissional", true) == 0)
		    {
		        ...
		        new company, post, reward, payday, jobid, counter;
	    ...
	    if(strlen(com_cmd) > 0)
	    {
			...
			else if(strcmp(com_cmd, "contratar", true) == 0)
			{
				...
				if(post == COMPANY_POST_EMPLOYEE)
				{
					...
					if(payday > 400000)
					{
					    SendClientMessage(playerid, COLOUR_GRAY, "Salário abaixo de $400000,00 dólares.");
					    return 1;
					}
				...
				SendPlayerInvite(plid, playerid, ((com & 0xF) << 24) | ((post & 0xF) << 20) | ((reward & 0xFFF) << 8) | ((payday / 100) & 0xFFF), INVITE_COMPANY, "Empresas", string, "Sim", "Nao");
				return 1;
			}
	...
	if(strcmp(cmd, "/veiculo", true) == 0)
	{
	    ...
	    if(strlen(vehicle_cmd) > 0)
	    {
			if(strcmp(vehicle_cmd, "comprar", true) == 0)
			{
			    ...
			    {
			        new string[256], owner[128], xpath[128];
	...
	if(strcmp(cmd, "/assinar", true) == 0)
	{
		...
		SetPlayerCounter(playerid, COUNTER_JOB, 720);
		...
		SendClientMessage(playerid, COLOUR_GREEN, "Contrato assinado! Você só poderá mudar de profissão novamente após 720 minutos (12h na realidade).");
		return 1;
	}
	...
	if(strcmp(cmd, "/pescar", true) == 0)
	{
		...
		{
			SendClientMessage(playerid, COLOUR_GRAY, "Você não está na Área de pesca, a área de pesca fica descendo o riozinho da Cooperativa de Pesca.");
			return 1;
		}
		...
		SetTimerEx("Fishery", 50000, false, "d", playerid);
		return 1;
	}
	...
	if(strcmp(cmd, "/venderpesca", true) == 0)
	{
		...
		GivePlayerSysMoney(playerid, fish);
	...
	if(strcmp(cmd, "/entregarbebidas", true) == 0)
	{
		...
		GivePlayerSysMoney(playerid, 20);
		SendClientMessage(playerid, COLOUR_BARMAN, "Local abastecido, você recebeu $20,00 dólares.");
	    return 1;
	}
	...
	if(strcmp(cmd, "/votekick", true) == 0)
	{
		...
		if( strcmp(name, "Saturnus", true) == 0 || strcmp(name, "Texugo", true) == 0 || strcmp(name, "michael", true) == 0 ||
            strcmp(name, "Brutows", true) == 0 || strcmp(name, "Manny", true) == 0 )
		{
	...
	if(strcmp(cmd, "/gps", true) == 0)
	{
	    ...
	    SendClientMessage(playerid, COLOUR_GRAY, "Escola Nautica, Escola Aerea, etc...");
	    return 1;
	}
	...
	if(strcmp(cmd, "/admin", true) == 0)
	{
	    ...
	    if(strcmp(adm_cmd, "admins", true) == 0)
	    {
	        new string[128];
	    ...
	    else
	    {
			...
			SendClientMessage(playerid, COLOUR_GRAY, "say, admins, report, banir, desbanir, kickar, arma, ir, trazer, empregar, skill, dinheiro,");
			SendClientMessage(playerid, COLOUR_GRAY, "vida, colete, paralisar, desparalisar, level, skin, veiculo, combustivel, cores, power,");
			SendClientMessage(playerid, COLOUR_GRAY, "spawnar, vspawnar, goto, anunciar, casa, vercasa, valorizarcasa, apagarcasa, server");
			SendClientMessage(playerid, COLOUR_GRAY, "mudarconta, apagarconta, varrercontas, resetarvenda, spec, voar, tapa, checar, fechar");
			return 1;
	    }
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == DIALOG_REGISTER_LOGIN)
	{
	    ...
	    else
	    {
			...
			SetPlayerHealth(playerid, pHealth[playerid]);
			SetPlayerArmour(playerid, hGetFloat(xpath, "Armour"));
...
public Introducing(playerid)
{
	...
	{
	    TextDrawSetString(tdintro[playerid], "Project Leader:~n~Saturnus");
	    time = 4000;
	}
	...
	{
	    TextDrawSetString(tdintro[playerid], "Virtual Society Team:~n~Texugo~n~michael~n~Brutows~n~Manny");
	    time = 4000;
	}
	...
	{
	    tdintro[playerid] = TextDrawCreate(320.0, 130.0, "~b~Virtual ~y~~h~Society ~w~R~y~~h~P~b~G");
	    TextDrawFont(tdintro[playerid], 1);
	    TextDrawLetterSize(tdintro[playerid], 1.8, 3.6);
		TextDrawAlignment(tdintro[playerid], 2);
		TextDrawColour(tdintro[playerid], COLOUR_WHITE);
		TextDrawShowForPlayer(playerid, tdintro[playerid]);
		time = 4000;
	...
	{
	    DrawTextForPlayer(playerid, "San Fierro - Cranberry Station - 1993", COLOUR_WHITE, TEXTDRAW_STYLE_5, 2000);
		SetPlayerSpecialAction(playerid, SPECIAL_ACTION_USECELLPHONE);
		time = 3000;
	}
	...
	{
		TextDrawSetString(tdintro[playerid], TEXT("~b~Pois é, sinto muito pelo que houve, ~n~vou sentir sua falta amigo..."));
	    time = 4000;
	}
	
	else if(IntroductionPart[playerid] == 19)
	{
		TextDrawSetString(tdintro[playerid], TEXT("Eu também vou, ser expulso de Liberty City, ~n~não poderia ter sido pior para mim..."));
	    time = 4000;
	}
	
	else if(IntroductionPart[playerid] == 20)
	{
		TextDrawSetString(tdintro[playerid], TEXT("~b~Ah cara, estes mafiosos vão pagar algum dia. ~n~Eai, já fez planos?"));
	    time = 4000;
	}
	
	else if(IntroductionPart[playerid] == 21)
	{
		TextDrawSetString(tdintro[playerid], TEXT("Sim, estou em San Fierro, vou pegar um avião e ~n~recomeçar minha vida em Los Santos."));
	    time = 4000;
	}
	...
	{
		TextDrawSetString(tdintro[playerid], TEXT("Eu também, Rafael... Até mais, amigo, mande ~n~abraços ao Jairo por mim!"));
	    time = 3000;
	}
	
	else if(IntroductionPart[playerid] == 24)
	{
		TextDrawSetString(tdintro[playerid], TEXT("~b~OK, até mais..."));
	    time = 2000;
	}
	...
	{
	    DrawTextForPlayer(playerid, "Valeu, parceiro.", COLOUR_WHITE, TEXTDRAW_STYLE_6, 1500);
	    RemovePlayerFromVehicle(playerid);
	    time = 4000;
	}
	...
	else if(IntroductionPart[playerid] == 32)
	{
        ...
        DissolveTextBlack(playerid, 0xFF);
		DrawTextForPlayer(playerid, "Los Santos International Airport - 1993", COLOUR_WHITE, TEXTDRAW_STYLE_5, 2000);
        time = 7500;
	}
...
public Text:CreateDrawText(content[], colour, style)
{
	...
	/*
		#define TEXTDRAW_STYLE_1                320.0, 130.0, 1, 0.7, 2.0, 1, 2
		#define TEXTDRAW_STYLE_2                318.0, 309.0, 1, 0.5, 1.5, 1, 2
		#define TEXTDRAW_STYLE_3                318.0, 309.0, 2, 0.4, 1.7, 1, 2
		#define TEXTDRAW_STYLE_4                318.0, 309.0, 1, 0.4, 1.7, 1, 2
		#define TEXTDRAW_STYLE_5                320.0, 130.0, 1, 0.5, 1.2, 1, 2
		#define TEXTDRAW_STYLE_6				370.0, 400.0, 1, 0.5, 1.5, 1, 2
		#define TEXTDRAW_STYLE_7                606.0, 126.0, 1, 0.3, 1.1, 1, 3
	*/
    ...
    switch(style)
	{
        ...
        case TEXTDRAW_STYLE_5:
		    X = 320.0, Y = 130.0, font = 1, sizeX = 0.5, sizeY = 1.2, shadow = 1, alignment = 2;
...
public Speedometer()
{
    for(new n = 0; n < MAX_SLOTS; n++)
	{
	    if(IsPlayerConnected(n) && IsPlayerLogged(n) && IsPlayerInAnyVehicle(n) && GetPlayerState(n) == PLAYER_STATE_DRIVER && tdspeed[n] != Text:INVALID_TEXT_DRAW)
	    {
	        new string[128];
			new vehid, model, F;
...
public Fishery(playerid)
{
	...
	if(rand <= 77)
	{
	    ...
	    format(string, sizeof string, "Você pescou um(a) %s de %dkg, esse peixe vale $%d por kilograma", Fishes[fish][fishName], mass, Fishes[fish][fishPrice]);
	    SendClientMessage(playerid, COLOUR_FISHER, string);
...
public Assaulting(playerid, textid)
{
	...
	if(strcmp(animlib, "ROB_BANK", false) != 0 || strcmp(animname, "CAT_Safe_Rob", false) != 0)
 	{
 	    point = GetSafePoint(safe);
...
public BusStop(playerid)
{
	...
	if(terminal == -1)
	{
	    ...
	    Pay(playerid, 50);
	    DrawTextForPlayer(playerid, "Rota concluída, você recebeu $50,00 dólares.", COLOUR_MACHINIST, TEXTDRAW_STYLE_2, 5000);
	}
        		...
        		for(new c = 1; c < sizeof Companies; c++)
        		{
    	    	    if(GetDistance(X, Y, Z, Companies[c][cX], Companies[c][cY], Companies[c][cZ]) <= 10.0)
	        	    {
	        	        ...
	        	        OPT[3] = (Jobs[jobid][jobHabA] == 1) ? ("Sim") : ("Não");
	       		        ...
	       		        format(string, sizeof string, "[INFORMACOES] Emprego: %s - Salário: $%d,00 dólares - Público: %s - Tempo de contrato: 720 minutos (12h reais)", Jobs[jobid][jobName], Jobs[jobid][jobPayday], OPT[0]);
	       		        SendClientMessage(n, COLOUR_WHITE, string);
			...
			if(GetPlayerInterior(n) == 0)
	        {
				...
				else
				{
					...
					if(House[n] != -1)
					{
				    	...
				    	tdhouse[n] = TextDrawCreate(23, 213, TEXT(msg));
			...
			else
			{
				...
				for(new z = 0; z < sizeof GangZones; z++)
				{
					    ...
					    Y <= GangZones[z][gzMaxY]   	)
					{
					    ...
					    tdzone[n] = TextDrawCreate(638.0, 395.0, "_");
...
public OnVehicleSpawn(vehicleid)
{
	...
	else
	{
	    ...
	    format(plate, sizeof plate, "PSR %d", vehicleid + 1000);
	    SetVehicleNumberPlate(vehicleid, plate);
	}
}
	...
	for(new n = 0; n < MAX_SLOTS; n++)
	{
	    if(IsPlayerConnected(n) && IsPlayerLogged(n))
	    {
	        ...
	        if(Audience[0] != n && Audience[1] != n && btime > 0)
	        {
				...
				result = 0;
				getdate(time[0][0], time[0][1], time[0][2]);
				...
				{
					format(string, sizeof string, "Mais um minuto se passou... Tempo restante: %dmins", btime - offset);
					SendClientMessage(n, COLOUR_WHITE, string);
				}
			}
