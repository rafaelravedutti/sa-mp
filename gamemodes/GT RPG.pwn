...
#define COLOR_MOTORISTA_PARTICULAR      0xFF9900AA
...
public OnPlayerRequestClass(playerid, classid)
{
	...
	GameTextForPlayer(playerid,"~g~please, login or register~n~   coder: DaMNeD", 5000,3);
	return 1;
}
...
public OnPlayerConnect(playerid)
{
	...
	GameTextForPlayer(playerid, "welcome to ~r~rpg! - ~n~coder: DaMNeD", 5000, 6);
	SendClientMessage(playerid, COLOR_SEGURANÇA, ".:GT:. RPG Mode");
	SendClientMessage(playerid, COLOR_WHITE, "Para ajuda inicial digite /ajuda");
	SendClientMessage(playerid, COLOR_WHITE, "Para os comandos do RPG digite /comandos e /comandos2");
	SendClientMessage(playerid, COLOR_WHITE, "Para uma breve explicação sobre o RPG digite /aprender");
	SendClientMessage(playerid, COLOR_WHITE, "Para as profissões do RPG digite /profs e /profs2");
	if (!udb_Exists(PlayerName(playerid))) {
		SendClientMessage(playerid, COLOR_WHITE, "Voce não está registrado, faça registro com /rregister [senha]");
	}
	if (udb_Exists(PlayerName(playerid))) {
    	format(msg,sizeof(msg),"Bem vindo novamente %s, digite /rlogin [senha] para se logar.",PlayerName(playerid));
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if(strcmp(cmd, "/rregister", true) == 0) {
		new tmp[256];
    	...
    	if (strlen(tmp)==0) {
			SendClientMessage(playerid, COLOR_RED, "/rregister [senha]");
			return 1;
		}
	...
	if(strcmp(cmd, "/rlogin", true) == 0 && IsPlayerConnected(playerid)) {
		new tmp[256];
		tmp = strtok(cmdtext, idx);
	    // The command shouldn't work if an account with this
	    ...
	    // Did he forgot the password?
	    if (strlen(tmp)==0) {
			SendClientMessage(playerid, COLOR_RED, "/rlogin [senha]");
			return 1;
		}
...
//--------------------------- COMANDOS DE VENDEDOR DE VEICULOS ----------------//
	...
	if(strcmp(cmd, "/setskin", true) == 0) {
		if(Profissao[playerid] == VENDEDOR_DE_CARROS || IsPlayerAdmin(playerid)){
			...
			tmp = strtok(cmdtext, idx);
			if(!strlen(tmp)){
				SendClientMessage(playerid, COLOR_RED, "/setskin [id] [amount]");
				return 1;
			}
		    plid = strval(tmp);
    	    tmp = strtok(cmdtext, idx);
...
//--------------------------- COMANDOS PARA TODOS OS PLAYERS -----------------//
    ...
    if(strcmp(cmd, "/ajuda", true) == 0) {
        SendClientMessage(playerid, COLOR_GREEN, ".:GT:. RPG");
        SendClientMessage(playerid, COLOR_YELLOW, "/comandos : comandos do RPG");
        SendClientMessage(playerid, COLOR_YELLOW, "/rregister [senha] : Se registra no jogo.");
        SendClientMessage(playerid, COLOR_YELLOW, "/rlogin [senha] : Se loga no jogo (Você tem que ter registro)");
	...
	if(strcmp(cmd, "/creditos", true) == 0) {
	    SendClientMessage(playerid, COLOR_GREEN, ".:GT:. RPG Mode");
	    SendClientMessage(playerid, COLOR_YELLOW, "Feito por: DaMNeD");
	    SendClientMessage(playerid, COLOR_YELLOW, "Agradecimentos:");
	    SendClientMessage(playerid, COLOR_YELLOW, "- Sedek e TheHitman (Idéias)");
		SendClientMessage(playerid, COLOR_YELLOW, "- Luigui (Alguns códigos)");
		SendClientMessage(playerid, COLOR_YELLOW, "- SuperX e f0zi(Alguns códigos)");
		SendClientMessage(playerid, COLOR_YELLOW, "- TheMaster, Sedek, Ciner e Eletron (Testes)");
		return 1;
	}

	if(strcmp(cmd, "/aprender", true) == 0) {
	    SendClientMessage(playerid, COLOR_GRAY, ".:GT:. RPG Mode");
		...
		SendClientMessage(playerid, COLOR_MOTORISTA, "Digite /ajuda ou /comandos para visualizar os comandos.");
		return 1;
	}
	
	if(strcmp(cmd, "/comandos", true) == 0) {
		SendClientMessage(playerid, COLOR_MOTORISTA, "Comandos do RPG - Parte 1");
		...
		SendClientMessage(playerid, COLOR_YELLOW, "/givecash : Envia dinheiro a algum jogador.");
        ...
        SendClientMessage(playerid, COLOR_MOTORISTA, "Para mais comandos, escreva /comandos2");
		return 1;
	}
	if(strcmp(cmd, "/comandos2", true) == 0) {
	    SendClientMessage(playerid, COLOR_MOTORISTA, "Comandos do RPG - Parte 2");
		...
		SendClientMessage(playerid, COLOR_YELLOW, "/say [texto] : Manda mensagem para pessoas proximas a você.");
		SendClientMessage(playerid, COLOR_YELLOW, "/relogio : Mostra o horário em San Fierro.");
		SendClientMessage(playerid, COLOR_YELLOW, "/profissao : Comandos de sua profissão.");
		SendClientMessage(playerid, COLOR_YELLOW, "/taxi [lugar] : Chama um taxi. | /banco : Comandos do Banco.");
		SendClientMessage(playerid, COLOR_YELLOW, "/buygas [litros] : Compra gasolina (você deve estar no posto para usar este comando).");
		SendClientMessage(playerid, COLOR_YELLOW, "/pagar [id] [quantidade] : Paga por algum serviço de corretor ou vendedor de carros.");
		SendClientMessage(playerid, COLOR_MOTORISTA, "Esses são os comandos do RPG.");
		return 1;
	}
	
	if(strcmp(cmd, "/banco", true) == 0) {
        SendClientMessage(playerid, COLOR_GREEN, "Comandos do Banco");
        SendClientMessage(playerid, COLOR_YELLOW, "/bdp [quantia] : Deposita dinheiro na sua conta do banco");
        SendClientMessage(playerid, COLOR_YELLOW, "/bwd [quantia] : retira dinheiro na sua conta do banco");
        SendClientMessage(playerid, COLOR_YELLOW, "/bstats : checa seu o saldo bancário..");
        SendClientMessage(playerid, COLOR_YELLOW, "/bscell : checa seu o saldo bancário através do seu celular, custo $20.");
        SendClientMessage(playerid, COLOR_YELLOW, "Você precisa estar em um banco para usar esses comandos, menos o /bscell.");
        SendClientMessage(playerid, COLOR_YELLOW, "Bancos: Hotel, Car Shop, San Fierro Train Station e SA Federal Mint.");
		return 1;
	}

	if(strcmp(cmd, "/profs", true) == 0) {
        SendClientMessage(playerid, COLOR_WHITE, "Profissões disponíveis no .:GT:. RPG : Parte 1");
    ...
    if(strcmp(cmd, "/profs2", true) == 0) {
        SendClientMessage(playerid, COLOR_WHITE, "Profissões disponíveis no .:GT:. RPG : Parte 2");
        ...
        SendClientMessage(playerid, COLOR_GRAY, "Estas são as profissões do .:GT:. RPG Mode.");
		return 1;
	}
    ...
    if(strcmp(cmd, "/buygas", true) == 0) {
		...
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)){
			SendClientMessage(playerid, COLOR_RED, "Uso: /buygas [litros]");
			return 1;
		}
	    quantidade = strval(tmp);
	    ...
	    if(IsPlayerConnected(i)) {
			if(X >= 1700.1732 && X <= 1741.9966 && Y >= -1721.5967 && Y <= -1605.9054){
			    ...
			    if(AreaPosto[i] == 0){
			        ...
			        SendClientMessage(i, COLOR_WHITE, "Posto para abastecer digite: /buygas [litros]");
					AreaPosto[i] = 1;
				}
			}
