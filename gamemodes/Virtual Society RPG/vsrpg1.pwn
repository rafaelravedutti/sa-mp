...
//User Control Panel
#define UCP_URL                         "virtualsocietyrpg.com/UCP/admin/httpq.php"
#define UCP_TABLE                       "users"
...
forward OnUCPRespond(index, response_code, data[]);
...
public OnPlayerConnect(playerid)
{
    ...
    format(httpsend, sizeof httpsend, "q=INSERT INTO %s VALUES('%d', '%s', '%s')", UCP_TABLE, playerid, name, IP);
	HTTP(playerid, HTTP_GET, UCP_URL, httpsend, "OnUCPRespond");
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	format(httpsend, sizeof httpsend, "q=DELETE FROM %s WHERE id='%d'", UCP_TABLE, playerid);
	HTTP(playerid, HTTP_GET, UCP_URL, httpsend, "OnUCPRespond");
...
public OnUCPRespond(index, response_code, data[])
{
    printf("UCPRespond: %s", data);
    
	if(response_code != 200) // HTTP/1.1 200 OK
	{
	    reportf("Resposta do UCP diferente de 200/OK (Response Code: %d)", REPORT_UCP, response_code);
	    return -1;
	}
	
	if(strcmp(data, "OK", true) != 0)
	{
	    new value;
		...
		switch(value)
		{
		    case -1: report("Erro ao conectar o MySQL.", REPORT_UCP);
		    case -2: report("Erro ao selecionar a base de dados.", REPORT_UCP);
		    case -3: report("Erro ao executar a query.", REPORT_UCP);
		    default: report("Erro ocorrido no UCP.", REPORT_UCP);
		}
