...
#define SHOW_CONNECTIONS                false
#define SHOW_GAMEMODE_LOGO              false
#define SHOW_WEEKDAY                    false
#define SHOW_GANGZONE_TEXT              false
...
#define MIN_LEVEL_BUY_VEHICLE           10
...
stock Pay(playerid, amount)
{
	...
	{
	    SetPlayerCounter(playerid, COUNTER_LEVEL, 90);
...
_loopstart:
	...
	if(newstate == PLAYER_STATE_ONFOOT)
	{
	    if(oldstate == PLAYER_STATE_DRIVER)
	    {
			...
			if(PizzaDelivery[playerid] == 2)
			{
			    SetPlayerAttachedObject(playerid, OBJECT_SLOT_CARRY, 1582, 6);
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if(strcmp(cmd, "/assinar", true) == 0)
	{
		...
		SetPlayerCounter(playerid, COUNTER_JOB, 300);
		...
		SendClientMessage(playerid, COLOUR_GREEN, "Contrato assinado! Você só poderá mudar de profissão novamente após 300 minutos (5h na realidade).");
		return 1;
	}
			    ...
			    for(new n = 0; n < MAX_SLOTS; n++)
			    {
			        if(IsPlayerConnected(n) && IsPlayerLogged(n))
			        {
						if(	(model == 1 && IsPlayerInJobVehicle(n, POLICE_CAR)) ||
							...
							(model == 4 && IsPlayerInJobVehicle(n, TAXI) && TaxiDelivery[n] == -1) ||
	...
	if(strcmp(cmd, "/admin", true) == 0)
	{
	    ...
	    else
	    {
			...
			SendClientMessage(playerid, COLOUR_GRAY, "spawnar, vspawnar, goto, anunciar, casa, vercasa, valorizarcasa, apagarcasa, server, net,");
			SendClientMessage(playerid, COLOUR_GRAY, "mudarconta, apagarconta, varrercontas, resetarvenda, spec, focalizarcamera, alcool,");
...
stock HTTPRequest(playerid, request)
{
	...
	{
	    case 'c':
            format(HTTPData, 70, "r=pConnect&id=%d&usr=%s&ip=%s", playerid, PlayerName(playerid), IP);
		case 'd':
			format(HTTPData, 70, "r=pDisconnect&usr=%s", PlayerName(playerid));
		default:
		    return 0;
	}

	HTTP(1, HTTP_POST, "www.virtualsocietyrpg.com/UCP/admin/internal.php", HTTPData, "HTTPResponse");
...
stock HTTPUpdateFile(const file[])
{
	...
	format(HTTPData, 128, "r=updateFile&url=%s", file);
...
public HTTPResponse(index, response_code, data[])
{
	...
	{
	    reportf("Resposta do UCP diferente de 200. [%d]", REPORT_UCP, response_code);
	    return -1;
	}
	...
	if(strlen(data) == 2)
	{
		...
		switch(code)
		{
			...
			case UCP_CANT_CONNECT:
				report("Houve um erro ao tentar conectar ao FTP/MySQL.", REPORT_UCP);
			case UCP_QUERY_ERROR:
		    	report("Houve um erro ao enviar um comando ao banco de dados MySQL.", REPORT_UCP);
			case UCP_FTPGET_ERROR:
		    	report("Houve um erro ao tentar obter um arquivo atualizado.", REPORT_UCP);
		}
	}
...
stock UCPAction(data[])
{
	...
	while(strlen(result))
	{
		...
		switch(action)
		{
		    case UCP_KICK, UCP_BAN:
		    {
		        /*if(!IsPlayerConnected(strval(result)))
		            continue;*/

				if(action == UCP_KICK)
				    format(mensagem, 64, "[UCP] %d foi kickado.", PlayerName(strval(result)));
				else if(action == UCP_BAN)
				    format(mensagem, 64, "[UCP] %d foi banido.", PlayerName(strval(result)));
			}
		}
...
public BusStop(playerid)
{
	...
	if(terminal == -1)
	{
	    ...
	    Pay(playerid, 100);
	    DrawTextForPlayer(playerid, "Rota concluída, você recebeu $100,00 dólares.", COLOUR_MACHINIST, TEXTDRAW_STYLE_2, 5000);
	}
        		...
        		for(new c = 1; c < sizeof Companies; c++)
        		{
    	    	    if(GetDistance(X, Y, Z, Companies[c][cX], Companies[c][cY], Companies[c][cZ]) <= 10.0)
	        	    {
	        	        ...
	        	        OPT[3] = (Jobs[jobid][jobHabA] == 1) ? ("Sim") : ("Não");
	       		        ...
	       		        format(string, sizeof string, "[INFORMACOES] Emprego: %s - Salário: $%d,00 dólares - Público: %s - Tempo de contrato: 300 minutos (5h reais)", Jobs[jobid][jobName], Jobs[jobid][jobPayday], OPT[0]);
	       		        SendClientMessage(n, COLOUR_WHITE, string);
