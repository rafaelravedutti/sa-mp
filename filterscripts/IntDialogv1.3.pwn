...
||		Last Edited: 31th January 2010 - Fixed Code - Much Better Now ...      				  ||
...
public OnFilterScriptInit()
{
	print("=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
	print("											  ");
	print("=> Interior Menu v1.3 By Garsino Loaded! <=");
	print("											  ");
	print("=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
	return 1;
}
...
{
	if(strcmp(cmdtext, "/interiormenu", true) == 0 || strcmp(cmdtext, "/intmenu", true) == 0)
	{
/* if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, RED, "Only Admins Can Use This Command!");
	...
	ShowPlayerDialog(playerid, INTERIORMENU, DIALOG_STYLE_LIST, "Interior Menu By [03]Garsino","24/7's\nAirports\nAmmunations\nHouses\nHouses 2\nMissions\nStadiums\nCasinos\nShops\nGarages\nGirlfriends\nClothing/Barber Store\nResturants/Clubs\nNo Category\nBurglary\nBurglary 2\nGym\nDepartment\nBack", "Select", "Cancel");
    return 1;
	}
	return 0;
}
